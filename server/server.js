...
try {
    ...
    const contractAdd = "0x42c8773d36274F1fa13d3f2d98378647F4a4e0Ac";
...
app.get('/getPatientReadings/:pid', async (req, res) => {
    try {
        await contract.methods.getPatientDetails(req.params.pid).call().then(function (value) {
            console.log(value);
            res.status(201).json({ success: true, ...value });
        }).catch(e => {
            const data = e.data;
